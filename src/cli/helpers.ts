...
/**
 * Ask user to pick from a list of options. Returns the selected option value.
 */
export function askChoice<T extends string>(
  ...
  return new Promise((resolve) => {
    ...
    options.forEach((opt, i) => {
      if (opt.value === defaultValue) defaultIdx = i;
    ...
    getRL().question(`${c.dim(`Enter choice [1-${options.length}]`)}: `, (answer) => {
      ...
      if (idx >= 0 && idx < options.length) {
        ...
        const match = options.find(o => o.value.toLowerCase() === a.toLowerCase() || o.label.toLowerCase() === a.toLowerCase());
...
export function detectPlatform(): 'macos' | 'linux' | 'wsl' {
  ...
  // Check for WSL
  try {
    const release = Bun.file('/proc/version');
    // We can't use textSync, so check synchronously via readFileSync
    const { readFileSync } = require('node:fs');
    const text = readFileSync('/proc/version', 'utf-8');
    if (text.toLowerCase().includes('microsoft') || text.toLowerCase().includes('wsl')) {
      return 'wsl';
    }
  } catch {
